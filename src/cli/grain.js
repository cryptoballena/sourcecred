...
import {encode, decode} from "../core/storage/textEncoding";
...
const grainCommand: Command = async (args, std) => {
  ...
  const diskStorage = new DiskStorage(process.cwd());
  ...
  const credResultJson = JSON.parse(
    decode(await diskStorage.get(credResultPath))
  );
  ...
  if (!simulation) {
    ...
    await accountsStorage.set("accounts.json", encode(stringify(credAccounts)));
  }
